...
import { Calendar as CalendarIcon, Search, Filter, RotateCcw, User, Edit, Plus, Trash2, Eye, EyeOff, MapPin, Tag } from 'lucide-react';
...
const getActionVisuals = (action: string) => {
    switch (action) {
        ...
        case 'CHANGE_VISIBILITY': return { icon: Eye, color: 'text-purple-600', label: 'Visibilidade' }; // Use Eye/EyeOff based on details
        // Add more actions as needed (e.g., INVENTORY, LOGIN, LOGOUT)
        default: return { icon: Edit, color: 'text-gray-500', label: action };
    }
};

const getEntityTypeVisuals = (type: string) => {
     switch (type) {
                    ...
                    <div className="pt-4 space-y-4 md:space-y-0 md:flex md:flex-wrap md:gap-4">
                        ...
                        <Popover>
                            ...
                            <PopoverContent className="w-auto p-0">
                                <Calendar mode="single" selected={filters.startDate} onSelect={(d) => handleFilterChange('startDate', d)} initialFocus locale={ptBR}/>
                            </PopoverContent>
                        </Popover>
                        <Popover>
                            <PopoverTrigger asChild>
                             ...
                             <PopoverContent className="w-auto p-0">
                                <Calendar mode="single" selected={filters.endDate} onSelect={(d) => handleFilterChange('endDate', d)} initialFocus locale={ptBR}/>
                            </PopoverContent>
                        </Popover>
                    ...
                    <Table>
                        ...
                        <TableBody>
                            ...
                            {!loading && logs.map((log) => {
                                const { icon: ActionIcon, color: actionColor, label: actionLabel } = getActionVisuals(log.action);
