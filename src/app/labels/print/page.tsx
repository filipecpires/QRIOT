...
export default function PrintLabelsPage() {
    ...
    const [filters, setFilters] = useState({
        search: '',
        category: '__all__', // Use __all__ as initial/default for 'all'
        location: '__all__', // Use __all__ as initial/default for 'all'
        page: 1,
        limit: 10,
    });
    ...
    const fetchData = async (currentFilters: typeof filters) => {
        setLoading(true);
        setError(null);
         // Map '__all__' back to empty string for API call if necessary
         const apiFilters = {
             ...currentFilters,
    ...
    const handleFilterChange = (key: keyof typeof filters, value: any) => {
        // Keep '__all__' in the state for Select components
    ...
    const generatePdf = async () => {
        ...
        doc.save(`etiquetas_qriot_${new Date().toISOString().slice(0,10)}.pdf`); // Updated filename prefix
        setIsGenerating(false);
                ...
                <CardHeader>
                    ...
                    <div className="pt-4 flex flex-wrap gap-4">
                        <Input
                            ...
                            className="max-w-sm"
                        />
                         {/* Use filters.category for Select value */}
                         <Select value={filters.category} onValueChange={(v) => handleFilterChange('category', v)}>
                            <SelectTrigger className="w-full md:w-[180px]">
                                <SelectValue placeholder="Categoria" />
                            </SelectTrigger>
                            <SelectContent>
                                {/* Use __all__ for the "All" option */}
                                <SelectItem value="__all__">Todas</SelectItem>
                                {categories.map(cat => <SelectItem key={cat} value={cat}>{cat}</SelectItem>)}
                            </SelectContent>
                        </Select>
                         {/* Use filters.location for Select value */}
